...
class TestTimeseriesManager:
    ...
    def test_read_tail_corrupt_lines(self, tmp_path):
        ...
        with mgr.path.open("w") as f:
            f.write("not json\n")
            f.write(json.dumps({"ts": 1, "total_bots": 1}) + "\n")
            f.write("[1,2,3]\n")  # array, not dict
        rows = mgr.read_tail(10)
        assert len(rows) == 1
...
class TestTrimToLatestEpoch:
    ...
    def test_single(self):
        rows = [{"total_turns": 100, "total_bots": 5}]
        assert TimeseriesManager.trim_to_latest_epoch(rows) == rows

    def test_no_epoch_boundary(self):
        rows = [
            {"total_turns": 100, "total_bots": 5},
            {"total_turns": 110, "total_bots": 5},
            {"total_turns": 120, "total_bots": 5},
        ]
        assert TimeseriesManager.trim_to_latest_epoch(rows) == rows

    def test_turn_drop_triggers_epoch(self):
        rows = [
            {"total_turns": 1000, "total_bots": 5},
            {"total_turns": 1050, "total_bots": 5},
            {"total_turns": 100, "total_bots": 5},  # big drop
            {"total_turns": 150, "total_bots": 5},
        ]
        result = TimeseriesManager.trim_to_latest_epoch(rows)
        assert len(result) == 2
        assert result[0]["total_turns"] == 100

    def test_bots_to_zero_triggers_epoch(self):
        rows = [
            {"total_turns": 100, "total_bots": 5},
            {"total_turns": 0, "total_bots": 0},
            {"total_turns": 10, "total_bots": 3},
        ]
        result = TimeseriesManager.trim_to_latest_epoch(rows)
        # Epoch boundary at index 1 (bots dropped to 0), so rows[1:] returned
        assert len(result) == 2
