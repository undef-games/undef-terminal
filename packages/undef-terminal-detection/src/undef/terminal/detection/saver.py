...
def _resolve_file_path(screens_dir: Path, filename: str, force: bool) -> Path:
    """Determine the final path to write a screen file to.

    If *force* is False the caller is expected to have already checked whether
    the hash was previously saved; this helper only handles the filename-
    collision logic for forced saves.

    Args:
        screens_dir: Directory in which the file will be written.
        filename: Base filename (already generated by the caller).
        force: When True, avoid overwriting an existing file by appending a
            ``-dup{N}`` suffix.

    Returns:
        Resolved :class:`~pathlib.Path` to write to.

    Raises:
        OSError: If *force* is True and no free filename can be found within
            10 000 attempts.
    """
...
class ScreenSaver:
    ...
    def save_screen(
        ...
        **Blocking I/O warning:** this method performs synchronous disk writes
        ...
        screen_file = _resolve_file_path(screens_dir, filename, force)
