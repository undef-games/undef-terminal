...
```bash
pip install undef-terminal-shell
```

## CLI

```bash
python -m undef.terminal.shell
```
...
```bash
pip install 'undef-terminal-shell[images]'   # installs Pillow
```
...
`undef-terminal-shell` is the engine behind the `ushell` connector type in
...
On Cloudflare Workers, `undef-terminal-cloudflare` vendors `undef-terminal-shell` into the
