...
import pytest
from undef.terminal.cloudflare.cli import build_parser, cmd_build, cmd_dev, main
...
class TestCmdBuild:
    ...
    def test_build_fails_when_entry_missing(self, tmp_path: Path) -> None:
        ...
        with patch("undef.terminal.cloudflare.cli._package_root", return_value=tmp_path):
            args = SimpleNamespace()
            result = cmd_build(args)
        assert result == 1
...
class TestCmdDevDeploy:
    def test_cmd_dev_calls_pywrangler(self) -> None:
        ...
        with (
            patch("undef.terminal.cloudflare.cli._require_pywrangler"),
            patch("undef.terminal.cloudflare.cli._run", return_value=0) as mock_run,
        ):
            result = cmd_dev(args)
        assert result == 0
    ...
    def test_cmd_deploy_calls_pywrangler(self) -> None:
        from undef.terminal.cloudflare.cli import cmd_deploy
...
class TestMain:
    def test_main_build_subcommand(self) -> None:
        with patch("undef.terminal.cloudflare.cli.cmd_build", return_value=0) as mock:
            result = main(["build"])
        assert result == 0
        mock.assert_called_once()

    def test_main_runtime_error_returns_1(self) -> None:
        with patch("undef.terminal.cloudflare.cli.cmd_build", side_effect=RuntimeError("oops")):
            result = main(["build"])
        assert result == 1
    ...
    def test_run_helper_invokes_subprocess(self) -> None:
        from undef.terminal.cloudflare.cli import _run
    ...
    def test_main_module_import(self) -> None:
        ...
        mod = importlib.import_module("undef.terminal.cloudflare.__main__")
...
class TestServeAssetFallbacks:
    def test_path_file_fallback_serves_existing_file(self, tmp_path: Path) -> None:
        ...
        from undef.terminal.cloudflare.ui import assets as assets_mod
