...
#
"""UshellConnector — SessionConnector implementation for ushell.
...
them.  A :class:`~undef.terminal.shell._repl.LineBuffer` accumulates them
...
try:
    from undef.terminal.server.connectors.base import SessionConnector as _SessionConnectorBase
except (ImportError, ModuleNotFoundError):
    from abc import ABC as _SessionConnectorBase  # type: ignore[assignment]
from undef.terminal.shell._commands import CommandDispatcher
from undef.terminal.shell._output import BANNER, PROMPT, term, worker_hello
from undef.terminal.shell._repl import LineBuffer
from undef.terminal.shell._sandbox import Sandbox


class UshellConnector(_SessionConnectorBase):  # type: ignore[misc]
    ...
    async def handle_input(self, data: str) -> list[dict[str, Any]]:
        ...
        for line in self._buf.take_completed():
            output_frames = await self._dispatcher.dispatch(line)
