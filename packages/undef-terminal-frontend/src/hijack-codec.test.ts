...
describe("ControlStreamDecoder", () => {
  describe("plain data", () => {
    it("returns a single data frame for plain text", () => {
      const dec = new ControlStreamDecoder();
  ...
  describe("error cases", () => {
    it("throws on invalid control prefix (DLE followed by non-STX non-DLE)", () => {
      const dec = new ControlStreamDecoder();
      expect(() => dec.feed("\x10X")).toThrow("invalid control stream prefix");
    });

    it("throws on non-hex characters in length field", () => {
      const dec = new ControlStreamDecoder();
      // DLE STX + 8 chars that are not valid hex + : + payload
      expect(() => dec.feed(`${_DLE}${_STX}ZZZZZZZZ:x`)).toThrow("invalid control stream length");
    });

    it("throws on missing colon separator", () => {
      const dec = new ControlStreamDecoder();
      // DLE STX + valid 8-hex + non-colon separator
      expect(() => dec.feed(`${_DLE}${_STX}00000001|x`)).toThrow("invalid control stream separator");
    });

    it("throws on payload exceeding max bytes", () => {
      const dec = new ControlStreamDecoder(10);
