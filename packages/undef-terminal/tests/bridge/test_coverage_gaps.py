...
from undef.terminal.bridge.rest_helpers import compile_expect_regex
from undef.terminal.bridge.worker_link import TermBridge
from undef.terminal.client import connect_test_ws
from undef.terminal.control_channel import ControlChannelProtocolError, encode_data
...
class TestWorkerWsBadStream:
    """Lines 112-116: Worker WS sends a message that fails control channel decode → close(1003)."""
...
class TestBrowserWsBadStream:
    """Lines 302-306: Browser WS sends a message that fails control channel decode → close(1003)."""
...
class TestBrowserHandlersResumeReclaimFail:
    ...
    async def test_resume_reclaim_fails_when_open_mode_active(self) -> None:
        ...
        """
        from undef.terminal.bridge.hub.resume import InMemoryResumeStore
        from undef.terminal.bridge.routes.browser_handlers import _handle_resume
