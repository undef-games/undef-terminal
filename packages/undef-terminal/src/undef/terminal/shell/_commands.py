...
class CommandDispatcher:
    ...
    async def dispatch(self, line: str) -> list[dict[str, Any]]:
        ...
        if cmd == "help":
            return self._handle_help(arg)

        if cmd == "clear":
            return [term("\x1b[2J\x1b[H" + PROMPT)]

        if cmd == "sessions":
            return await self._dispatch_sessions(arg)

        result = await self._dispatch_named_cmd(cmd, arg)
        if result is not None:
            return result
        return [term(error_msg(f"unknown command: {cmd!r} — type {BOLD}help{RESET}") + PROMPT)]

    def _handle_help(self, arg: str) -> list[dict[str, Any]]:
        """Return help output for the help command."""
        if arg:
    ...
    async def _dispatch_named_cmd(self, cmd: str, arg: str) -> list[dict[str, Any]] | None:
        """Dispatch to a named command handler. Returns None if cmd is unknown."""
        if cmd == "py":
    ...
    async def _dispatch_sessions(self, arg: str) -> list[dict[str, Any]]:
        """Dispatch sessions sub-commands (list or kill)."""
    ...
    async def _cmd_storage(self, arg: str) -> list[dict[str, Any]]:
        ...
        if sub == "list":
            return await self._cmd_storage_list(storage)
        if sub == "get":
            return await self._cmd_storage_get(storage, key_arg)
        return [term(error_msg("usage: storage list | storage get <key>") + PROMPT)]

    async def _cmd_storage_list(self, storage: Any) -> list[dict[str, Any]]:
        try:
    ...
    async def _cmd_storage_get(self, storage: Any, key_arg: str) -> list[dict[str, Any]]:
        if not key_arg:
    ...
    async def _cmd_kv(self, arg: str) -> list[dict[str, Any]]:
        ...
        if sub == "list":
            return await self._cmd_kv_list(kv)
        if sub == "get":
            return await self._cmd_kv_get(kv, key_arg)
        if sub == "set":
            return await self._cmd_kv_set(kv, key_arg)
        if sub == "delete":
            return await self._cmd_kv_delete(kv, key_arg)
        return [term(error_msg("usage: kv list | kv get <key> | kv set <key> <value> | kv delete <key>") + PROMPT)]

    async def _cmd_kv_list(self, kv: Any) -> list[dict[str, Any]]:
        try:
    ...
    async def _cmd_kv_get(self, kv: Any, key_arg: str) -> list[dict[str, Any]]:
        if not key_arg:
    ...
    async def _cmd_kv_set(self, kv: Any, key_arg: str) -> list[dict[str, Any]]:
        if not key_arg:
    ...
    async def _cmd_kv_delete(self, kv: Any, key_arg: str) -> list[dict[str, Any]]:
        if not key_arg:
    ...
    async def _cmd_fetch(self, arg: str) -> list[dict[str, Any]]:
        ...
        parsed = self._parse_fetch_args(arg)
        if parsed is None:
            return [term(error_msg("usage: fetch [-X METHOD] <url> [body]") + PROMPT)]
        method, url, body = parsed
        try:
            status, text = await self._do_fetch_request(method, url, body)
    ...
    @staticmethod
    def _parse_fetch_args(arg: str) -> tuple[str, str, str | None] | None:
        """Parse fetch argument string into (method, url, body). Returns None on usage error."""
        method = "GET"
        rest = arg
    ...
    async def _do_fetch_request(self, method: str, url: str, body: str | None) -> tuple[int, str]:
        """Execute an HTTP request via js.fetch (CF worker) or urllib fallback."""
        try:
